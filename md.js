...
sock.sendMessage(from, { caption: anj, location: { jpegThumbnail: ktedh }, templateButtons: button, footer: `Creator Zanz`, mentions: [m.sender] })
.catch ((err) => reply(err))
}
...
const botzkir = "6282147142709@s.whatsapp.net"
...
Saya Zanz Executed Saya Disini Ingin Membantu Kalian Membuat Sticker Download Video/Lagu Youtube, Tiktok Dll.`
let buttoons = [
...
*╭──「 Stalker 」*
☻︎ ${prefix}ffstalk SES>666Zanz
☻︎ ${prefix}ghstalk Denx
☻︎ ${prefix}igstalk denikrnww
...
*╰─────────────*`
let btn = []
sock.send5ButLoc(m.chat, memegnu, `Creator Zanz`, tod, btn)
}
addCmd(command.slice(0), 1, commund)
break
case 'animes':{
...
await sock.send5ButLoc(from, txt , `Creator Zanzz+++`,gam, but , { userJid: m.chat, quoted: m })
})
.catch((err) => {
reply(err)
})
...
"url": `https://wa.me/6282147142709`
},
"businessOwnerJid": "6282147142709@s.whatsapp.net",
}
...
waktu = q.split('|')[2]
for (let i = 0; i < jumlah; i++) {
sock.sendMessage(num, { text: 'MAMPUS KONTOL GW Zanz' }, { quoted: lep})
await sleep(ms(waktu))
}
...
await sock.sendButtonText(m.chat, buttonns, `Mode Antionce`, `_Creator Zanz_`, m)
}
addCmd(command.slice(0), 1, commund)
break
case 'antidelete': {
...
await sock.sendButtonText(m.chat, buttons, `Mode Antilink`, `_Creator Zanz_`, m)
}
addCmd(command.slice(0), 1, commund)
break
case 'tagall': {
...
const djej = "6282147142709@s.whatsapp.net"
